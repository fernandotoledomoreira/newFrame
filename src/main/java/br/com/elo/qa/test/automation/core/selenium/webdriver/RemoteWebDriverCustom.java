...
public class RemoteWebDriverCustom extends RemoteWebDriver implements WebDriver, JavascriptExecutor, FindsById,
		FindsByClassName, FindsByLinkText, FindsByName, FindsByCssSelector, FindsByTagName, FindsByXPath,
		HasInputDevices, HasCapabilities, Interactive, TakesScreenshot, Command {

	private RemoteWebDriver driver;
	private int timeout;

	public RemoteWebDriverCustom(RemoteWebDriver driver, int timeout) {
		this.driver = driver;
	...
	public Boolean waitProcessPage() {
		...
		return webDriverWait.until(waitProcessAngularJs()) && webDriverWait.until(waitProcessJQuery())
				&& webDriverWait.until(waitProcessJavaScript()) && webDriverWait.until(waitProcessAjax());
	}
	...
	public int getCurrentScrollY() {
		return ((Number) executeJs("var scrY = window.scrollY; if(scrY){return scrY;} else {return 0;}")).intValue();
	}

	@Override
	public int getCurrentScrollX() {
		return ((Number) executeJs("var scrX = window.scrollX; if(scrX){return scrX;} else {return 0;}")).intValue();
	}
	...
	public WebElement castElement(WebElement element) {
		return new WebElementCustom(element, this);
	}

	@Override
	public List<WebElement> castElement(List<WebElement> element) {
		List<WebElement> ret = new ArrayList<>();
		element.forEach(p -> ret.add(new WebElementCustom(p, this)));
		return ret;
	}
	...
	private ExpectedCondition<Boolean> waitProcessAjax() {
		ExpectedCondition<Boolean> ajaxLoad = new ExpectedCondition<Boolean>() {
			@Override
			public Boolean apply(WebDriver driver) {
				try {
					String js = "if (typeof window.Ajax != 'undefined') {\r\n"
							+ " return window.Ajax.activeRequestCount; \r\n"
							+ "} else {return 0;}";
					
					return Long.parseLong(((JavascriptExecutor) driver)
							.executeScript(js).toString()) == 0;
				} catch (Exception e) {
					return true;
				}
			}
